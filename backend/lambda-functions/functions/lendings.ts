...
import { LendingsController } from 'controllers/LendingsController';
import {
    HttpMethod,
    HttpStatus,
...
const handler = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
    try {
        ...
        const controller = new LendingsController(userId);
        const path = event.resource;
        const body = event.body ? JSON.parse(event.body) : {};

        // Payment routes
        if (path === '/lendings/payments') {
            switch (event.httpMethod) {
                case HttpMethod.GET: {
                    const lendingTimestamp = event.queryStringParameters?.lendingTimestamp;
                    if (!lendingTimestamp) {
                        return createBadRequestResponse(HttpStatus.BAD_REQUEST, 'lendingTimestamp query parameter is required.');
                    }
                    return await controller.getPayments(lendingTimestamp);
                }
                case HttpMethod.POST:
                    return await controller.postPayment(body);
                default:
